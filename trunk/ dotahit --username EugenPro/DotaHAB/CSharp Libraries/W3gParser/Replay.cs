...
namespace Deerchao.War3Share.W3gParser
{
    public class Replay
    {
        ...
        private int gameStart = 0; //! added fields
        ...
        private void Load(Stream stream)
        {
            ...
            // make action times to be the same as ingame actions
            if (gameStart !=0 )
                foreach (Player player in players)
                {
                    foreach (OrderItem item in player.Items.BuildOrders)
                    {
                        item.Time -= gameStart;
                        if (item.Time < 0) item.Time = 0;
                    }

                    Hero H = player.GetMostUsedHero();
                    if (H != null) 
                    foreach (OrderItem skill in H.Abilities.BuildOrders)
                    {
                        skill.Time -= gameStart;
                        if (skill.Time < 0) skill.Time = 0;
                    }

                    player.Time -= gameStart;
            ...
            while (rest > 0)
            {
                ...
                while (prest > 0)
                {
                    ...
                    switch (actionId)
                    {
                        ...
                        case 0x60:
                            //unknownA, unknownB
                            reader.ReadInt64(); //! gamemode detection
                            long pos = reader.BaseStream.Position;
                            string command = ParserUtility.ReadString(reader).ToLower(); //make sure it's mode command
                        ...
                        case 0x6B:
                            ...
                            switch (missonKey)
                            {
                                ...
                                case "11":
                                    ...
                                    if (key == "9")
                                    {
                                        string codeid = ParserUtility.StringFromUInt((uint)value);
